 ...
 * upd: -1
 *
 * Перепишите кто-нибудь это дерьмо!
 */
...
var Search = {
	...
	RequestPage: function (opts) {
		...
		Elem.id = "search-form";
		Elem.appendChild(Site.CreateHeader("Поиск" + (Site.Get("group_id") ? " по группе" : ""), "<span class='i' id='search-count'></span>"));
	...
	Listing: function (data) {
		...
		if (count === 0)
			newList.appendChild(Site.EmptyField("По заданным параметрам ничего не найдено"));
		newList.appendChild(Site.PagebarV2(Site.Get("offset"), count > 1000 ? 1000 : count, 30));
