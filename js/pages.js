var Pages = {
	...
	load: function(opts) {
		...
		api("execute", {
			code: ("var p=" + (!opts.version ? "API.pages.get({owner_id:%o,title:\"%t\",page_id:\"%i\",v:5.24,need_html:1,need_source:1%s%g})" : "API.pages.getVersion({group_id:%o,version_id:%v,need_html:1})") + ";return{page:p,group:API.groups.getById({group_id:p.group_id,v:5.24}),users:API.users.get({user_ids:[p.creator_id,p.editor_id],v:5.24})};")
	...
	showContent: function(page) {
		...
		if (page.canEdit) {
			actions[Lang.get("pages.action_edit")] = function() {
				Pages.editPage(page);
			};
		}

		if (page.canEditAccess) {
			actions[Lang.get("pages.action_edit_access")] = function() {
				Pages.setAccessControl(page, content);
			};
		}

		actions[Lang.get("pages.action_show_versions")] = function() {
			window.location.hash = "#page-" + page.groupId + "_" + page.pageId + "?act=versions";
		};

		Wiki.insertStyles();

		parent.appendChild(Site.getPageHeader(page.getTitleString(), page.canEdit ? Site.CreateDropDownMenu(Lang.get("general.actions"), actions) : null));
	...
	editPage: function(page) {
		var e = $.e,
			p = e("div", {append: Site.getPageHeader(Lang.get("pages.head_edit"))}),
	...
	save: function(page, source, button) {
	...
	showHistory: function(items, groupId, pageId) {
		var e = $.e,
			p = e("div", {append: Site.getPageHeader(Lang.get("pages.head_history"))}),
	...
	getTitles: function(groupId) {
		api("pages.getTitles", {group_id: groupId > 0 ? groupId : -groupId, v: 5.24}).then(function(data) {
			var e = $.e,
				parent = e("div"),
				list = e("div");
			...
			parent.appendChild(Site.getPageHeader(Lang.get("pages.head_all_pages_of_group"), data.length + " " + Lang.get("pages", "pages", data.length)));
...
function Page(p, noCache) {
	...
	self.getTitleString = function() {
		...
		return $.e("span", {append: [
			$.e("a", {href: g.link, html: g.name.safe()}),
			document.createTextNode(" » "),
			$.e("a", {href: "#page-" + this.groupId + "_" + this.pageId, html: this.title.safe()})
		]});
	};

	self.getAuthor = function() {
		var u = Local.data[this.creatorId];
		return {name: getName(u), link: "#" + (u.screen_name || "id" + u.id)};
	};

	self.getEditor = function() {
		var u = Local.data[this.editorId];
		return {name: getName(u), link: "#" + (u.screen_name || "id" + u.id)};
	};
