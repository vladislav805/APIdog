...
var Dev = {
	...
	inited: false,
	init: function () {
		document.getElementsByTagName("head")[0].appendChild($.e("link", {rel: "stylesheet", href: "/includes/dev.css"}));
		Dev.inited = true;
	},
	explain: function (url) {
		...
		if (!Dev.inited)
			Dev.init();
		Site.Loader();
		Support.Ajax("/api/v2/apidog.getVKDevPage", {
			p: url
		}, function (data) {
			data = data.response;
	...
	show: function (page) {
		...
		w.appendChild(Site.CreateHeader(page.title));
