	...
	parseContent: function(text) {
		return text
			...
			.replace(/href="(https?:\/\/)?[A-Za-z0-9.]*(vk\.com|vkontakte\.ru|apidog\.ru)?\/?([^"]*)"/img, function(all, protocol, domain, path) {
				return "href=\"" + (/away.php/img.test(all) ? decodeURI(path.replace(/\/?away(\.php)?\?to=/img, "")) : "#" + path) + "\"";
			})
			.replace(/\n/mg, "<br \/>");
	},
