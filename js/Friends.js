...
//					return Friends.showFriends(ownerId, Friends.friends[ownerId]);
	...
	showFriendsOnPage: function (user_id, response) {
		var count = response.count,
			...
			actions = null,
			empty = Site.EmptyField(Lang.get("friends.no_friends")),
			to = Photos.getToParam(1);
		if (list_id && !to)
		...
		head.appendChild(Site.CreateHeader(count + " " + $.textCase(count, Lang.get("friends.friend_s")), actions ? actions : false));
		head.appendChild(Friends.showSearchForm(friends));
		Local.AddUsers(friends);
	...
	showRequests: function (data) {
		...
		if (data.length)
			...
			parent.appendChild(Site.EmptyField("Заявок нет"));
	...
	getMutual: function (owner_id) {
		Site.API("execute",{
			...
			parent.appendChild(Site.CreateHeader(data.length + " " + $.TextCase(data.length, Lang.get("friends.mutual_friends"))));
	...
	editList: function (listId)
	{
		...
		for (var i = 0, l = friends.length; i < l; ++i) {
			...
			list.appendChild(e("label", {"class": "miniprofiles-item" + (hasInList ? " mp-checked" : ""), append: [
				...
				e("div", {"class": "miniprofiles-right", append: [e("strong", {
					"class": "a",
					html: Site.Escape(friends[i].first_name + " " + friends[i].last_name) + Site.isOnline(friends[i])
				})]})
			], onclick: function (event) {
				var check = this.querySelector("input[type=checkbox]");
				if (check.checked)
		...
		parent.appendChild(Site.CreateHeader(Lang.get("friends.creating_list")));
	...
	getCalendar: function () {
		...
		parent.appendChild(Site.CreateHeader("Календарь"));
